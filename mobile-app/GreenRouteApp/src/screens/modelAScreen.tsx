...
export default function ModelAScreen() {
  ...
  const runForecast = async () => {
    ...
    setLoading(true);
    try {
      await axios.get(`http://192.168.29.190:5786/api/model-a/run?product_id=${productId}`);
      const chart = `http://192.168.29.190:5786/api/model-a/chart?ts=${Date.now()}`;
      const csv = await axios.get(`http://192.168.29.190:5786/api/model-a/data`);
