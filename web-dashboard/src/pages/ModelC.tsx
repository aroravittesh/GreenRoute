...
export default function DistributionPlanner() {
  ...
  useEffect(() => {
    ...
    if (savedCsv && savedTimestamp) {
      ...
      setChartUrl(`https://sanchaya.work.gd/api/model-c/image?ts=${savedTimestamp}`);
    }
  }, []);
  ...
  const runOptimizer = async () => {
    setLoading(true);
    try {
      await axios.get("https://sanchaya.work.gd/api/model-c/run");

      const timestamp = Date.now();
      setChartUrl(`https://sanchaya.work.gd/api/model-c/image?ts=${timestamp}`);
      localStorage.setItem(STORAGE_KEY_TIMESTAMP, timestamp.toString());

      const csv = await axios.get("https://sanchaya.work.gd/api/model-c/predictions");
