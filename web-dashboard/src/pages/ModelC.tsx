...
export default function DistributionPlanner() {
  ...
  useEffect(() => {
    ...
    if (savedCsv && savedTimestamp) {
      ...
      setChartUrl(`http://54.197.3.23:5786/api/model-c/image?ts=${savedTimestamp}`);
    }
  }, []);
  ...
  const runOptimizer = async () => {
    setLoading(true);
    try {
      await axios.get("http://54.197.3.23:5786/api/model-c/run");

      const timestamp = Date.now();
      setChartUrl(`http://54.197.3.23:5786/api/model-c/image?ts=${timestamp}`);
      localStorage.setItem(STORAGE_KEY_TIMESTAMP, timestamp.toString());

      const csv = await axios.get("http://54.197.3.23:5786/api/model-c/predictions");
