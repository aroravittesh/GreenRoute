...
exports.default = SpoilageForecast;
...
function SpoilageForecast() {
    ...
    (0, react_1.useEffect)(function () {
        var savedChartUrl = localStorage.getItem("chartUrl");
        var savedCsvData = localStorage.getItem("csvData");
        if (savedChartUrl)
            setChartUrl(savedChartUrl);
        if (savedCsvData)
            setCsvData(JSON.parse(savedCsvData));
    }, []);
    var runSimulation = function () { return __awaiter(_this, void 0, void 0, function () {
        var imageUrl, csv, parsed, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, axios_1.default.get("https://sanchaya.live/api/model-b/run")];
                case 2:
                    _a.sent();
                    imageUrl = "https://sanchaya.live/api/model-b/image?ts=".concat(Date.now());
                    setChartUrl(imageUrl);
                    localStorage.setItem("chartUrl", imageUrl);
                    return [4 /*yield*/, axios_1.default.get("https://sanchaya.live/api/model-b/predictions")];
                case 3:
                    csv = _a.sent();
                    parsed = csv.data
                        .split("\n")
      ...
      <framer_motion_1.motion.div key="home" initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -30 }} transition={{ duration: 1 }} style={{ padding: "2rem" }}>
        <main className="pt-24 px-6 max-w-7xl mx-auto">
          <div className="mb-10">
            ...
            <p className="text-gray-600 mb-4">Forecast vs Actual Sales for 30 Days</p>
            <button onClick={runSimulation} className="bg-green-700 text-white px-6 py-2 rounded-md hover:bg-green-800 transition">
              {loading ? "Running..." : "Run Forecast"}
            </button>
          </div>
          ...
          {chartUrl && (<div className="mb-10">
              <h2 className="text-xl font-medium text-gray-800 mb-3">Forecast Chart</h2>
              <div className="bg-white border rounded-lg shadow p-4">
                <img src={chartUrl} alt="Spoilage Forecast" className="w-full max-h-[480px] object-contain rounded" style={{ border: "1px solid #e0e0e0" }}/>
              </div>
            </div>)}
          ...
          {csvData.length > 1 && (<div className="mb-16">
              ...
              <div className="overflow-x-auto bg-white rounded-lg shadow border">
                <table className="min-w-full table-auto text-sm">
                  <thead className="bg-gray-100 text-gray-600 font-medium">
                    <tr>
                      {csvData[0].map(function (header, idx) { return (<th key={idx} className="px-6 py-3 text-left">{header}</th>); })}
                    </tr>
                  </thead>
                  <tbody className="text-gray-700">
                    {csvData
                .slice(1)
                .filter(function (row) { return row.length >= 2 && !isNaN(parseFloat(row[1])); })
                .map(function (row, idx) { return (<tr key={idx} className="border-t">
                          {row.map(function (cell, i) { return (<td key={i} className="px-6 py-3">{cell}</td>); })}
                        </tr>); })}
                  </tbody>
                </table>
              </div>
